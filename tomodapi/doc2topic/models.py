...
class Doc2Topic:
	...
	def build(self, corpus, n_topics=20, batch_size=1024*6, n_epochs=5, lr=0.015, l1_doc=0.000002, l1_word=0.000000015, word_dim=None, generator=None):
		init_tf_memory()
		self.corpus = corpus
	...
	def get_wordvecs(self, min_zero=True):
		try:
			...
			inlayerW = Input((1,))
			embW = Embedding(vocab_len, word_dim, input_length=1, weights=self.model.layers[self.layer_lookup['wordemb']].get_weights())(inlayerW)
			embWa = Dense(n_topics, activation='relu', weights=self.model.layers[self.layer_lookup['wordproj']].get_weights())(embW)
	...
	def get_topic_words(self, top_n=10, stopwords=set()):
		...
		for topic in range(self.wordvecs.shape[1]):
			...
			topic_words[topic] = [(self.idx2token[str(idx)], score) for idx, score in topic_words[topic] if self.idx2token[str(idx)] not in stopwords]
